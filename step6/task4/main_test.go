...
func TestCompareStudents(t *testing.T) {
	var tests = []struct {
		name     string
		student  Student
		other    Student
		...
		{
			name:     "Case student greater than other",
			student:  Student{"Aaaa", 20},
			other:    Student{"Bbbb", 10},
			expected: ">",
		},
		{
			name:     "Case student less than other",
			student:  Student{"Aaaa", 10},
			other:    Student{"Bbbb", 20},
			expected: "<",
		},
		{
			name:     "Case student equal other",
			student:  Student{"Aaaa", 20},
			other:    Student{"Bbbb", 20},
			expected: "=",
		},
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			got := compareStudents(test.student, test.other)

			if got != test.expected {
				t.Errorf("unexpected value for %v, %v: got %v, expected %v\n", test.student, test.other, got, test.expected)
			}
		})
	}
}
...
func TestCompareList(t *testing.T) {
	var tests = []struct {
		...
		{
			name:         "Case one student",
			studentNames: []string{"Bob50"},
			expected:     map[string]string{"Bob50": "="},
		},
		{
			name:         "Case many students",
			studentNames: []string{"Sara60", "Jack50", "John40", "Den10"}, // avg: 37.5 -> 37
			expected: map[string]string{
				"Sara60": ">",
				"Jack50": ">",
				"John40": "=",
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			got, err := CompareList(test.studentNames)
