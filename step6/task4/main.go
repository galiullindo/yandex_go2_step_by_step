...
func compareStudents(student Student, other Student) string {
	if student.Mark > other.Mark {
		return ">"
	}
	if student.Mark < other.Mark {
		return "<"
...
func CompareList(names []string) (map[string]string, error) {
	...
	studentsMap := make(map[string]string)
	averageStudent := Student{Mark: mark}
	for _, student := range students {
		value := compareStudents(student, averageStudent)
		studentsMap[student.Name] = value
	}
